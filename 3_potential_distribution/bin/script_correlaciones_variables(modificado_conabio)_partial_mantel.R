...
setwd("/media/nel_pc/n311_pc/MAPA_QUERCUS_MAC/conabio/todas_capas_pais_ascii_cortes/Nueva carpeta/2_corte_pais_eda_asc/")
...
class(presencias_clima3)
head(presencias_clima3)
...
### Guardar todo el data.frama
write.csv(presencias_clima3, file = "q_macd_79puntos_bioclimas.csv") ### para guardar el archivo de puntos

dist_21_bio18 <- dist(presencias_clima3[,21])
dist_simil_18 <- proxy::simil(presencias_clima3[,21])

dist_14_bio11 <- dist(presencias_clima3[,14])
dist_simil_11 <- proxy::simil(presencias_clima3[,14])

dist_19bio <- dist(presencias_clima3[,4:22])
dist_simil_bio19 <- proxy::simil(presencias_clima3[,4:22])

dist_simil_bio_8 <- proxy::simil(presencias_clima3[,c(4,6,11,12,14,19,21,22)])
